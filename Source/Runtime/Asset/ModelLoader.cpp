...
void ModelLoader::SetSemanticAttribute(VertexAttribute::Semantic semantic)
{
    m_allowedSemantics.insert(semantic);
}
...
void ModelLoader::GenerateSubmesh(Mesh& mesh, const aiMesh& meshData)
{
    ...
    std::vector<GLubyte> vertexData = CollectVertexData(meshData, vertexFormat, interleaved, m_allowedSemantics);
...
std::vector<GLubyte> ModelLoader::CollectVertexData(const aiMesh& meshData, VertexFormat& vertexFormat, bool interleaved, std::set<VertexAttribute::Semantic> allowedSemantics)
{
    vertexFormat.Clear();
